...
class CommandPanel: public wxPanel
{
    public:
        ...
        /// Sends input command to the widget.
        /// The commands are shown in a history text box
        ///\param txt command with arguments
        DLL_EXPORT void InputText(const wxString &txt);

        /// <summary>
        /// Send message to show in the panel
        ///\param message
        DLL_EXPORT void SendMessage(const wxString &txt);

        /// Assign command list for an autocomplete util.
        /// Works if tab-key is pressed during the input (for the first word only)
